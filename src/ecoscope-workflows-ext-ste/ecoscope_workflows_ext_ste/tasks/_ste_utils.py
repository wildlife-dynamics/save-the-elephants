...
def generate_speed_raster(
    ...
    resolution: Annotated[
        Optional[float], Field(default=None, description="Resolution of the raster. If None, mean of dist_col is used.")
    ] = None,
    ...
    ecograph = Ecograph(
        Trajectory(gdf), resolution=res, radius=radius, cutoff=cutoff, tortuosity_length=tortuosity_length
    )
...
def create_seasonal_labels(
    ...
    try:
        ...
        season_bins = pd.IntervalIndex(seasonal_wins.apply(lambda x: pd.Interval(x["start"], x["end"]), axis=1))
        labels = seasonal_wins["season"]

        traj["season"] = pd.cut(traj["segment_start"], bins=season_bins).map(dict(zip(season_bins, labels)))
